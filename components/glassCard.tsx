...
function GlassCard({ name, status, top, left, uptime, latency, lastUpdated, history, className, mouseContainerRef, icon }: GlassCardProps) {
        ...
        <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-3">
                <div
                    ...
                    isHealthy ? "bg-emerald-500/80 backdrop-blur animate-pulse" : "bg-amber-500/80 backdrop-blur animate-pulse"
                    }`}
                    title={statusTranslation[status as keyof typeof statusTranslation]}
                >
                </div>
                <div>
                    <p className="text-sm text-white/90">{statusTranslation[status as keyof typeof statusTranslation]}</p>
                </div>
            </div>
        </div>
        <div className="space-y-3">
          <div className="pt-2 space-y-2">
            <div className="flex justify-between">
              ...
              <span className="text-sm">{statusTranslation[status as keyof typeof statusTranslation]}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-white/90">Uptime:</span>
              <span className="text-sm">{typeof uptime === "number" ? `${uptime}%` : uptime}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-white/90">Latência:</span>
              <span className="text-sm">{typeof latency === "number" ? `${latency}ms` : latency}</span>
            </div>
            <div className="flex justify-between">
  ...
  if (!ready) {
    ...
    return <div ref={ownRef} className={`relative z-10 w-full min-h-screen h-screen overflow-hidden ${className ?? ""}`} />;
  }
